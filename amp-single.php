...
<html amp>
  <head>
	...
	if(!empty($adsid) && !empty($adslot)){
							$konten.= '<script async custom-element="amp-ad" src="https://cdn.ampproject.org/v0/amp-ad-0.1.js"></script>';
	};
	if(!empty($analytics)){
		$konten.= '<script custom-element="amp-analytics" src="https://cdn.ampproject.org/v0/amp-analytics-0.1.js" async></script>';
	};
	$konten.= '
...
.amp-hide {
  ...
  <body>';
if(!empty($analytics)){
	$konten.='<amp-analytics type="googleanalytics">
		<script type="application/json">
		{
		  "vars": {
			"account": "<?php echo get_option("zaenu_analytics")?>"
		  },
		  "triggers": {
			"trackPageview": {
			  "on": "visible",
